...
class IPv4
  ...
  public:
    ...
    constexpr IPv4(unsigned char aPart0, unsigned char aPart1,
                  unsigned char aPart2, unsigned char aPart3)
    ...
    constexpr unsigned long toUlong() const noexcept
      {
      return (static_cast<unsigned long>(theData[0]) << 24) |
            (static_cast<unsigned long>(theData[1]) << 16) |
            (static_cast<unsigned long>(theData[2]) << 8) |
            (static_cast<unsigned long>(theData[3]));
      }
    ...
    friend bool operator<(IPv4 const &aLeft, IPv4 const &aRight) noexcept
      {
    ...
    friend std::ostream &operator<<(std::ostream &aOstream, const IPv4 &aAddress)
      ...
      aOstream << static_cast<int>(data[0]) << "."
    ...
    friend std::istream &operator>>(std::istream &aIstream, IPv4 &aAddress)
      {
      char data1, data2, data3;
      int bit1, bit2, bit3, bit4;

      aIstream >> bit1 >> data1 >> bit2 >> data2 >> bit3 >> data3 >> bit4;
      if ((data1 == '.') && (data2 == '.') && (data3 == '.'))
        aAddress = IPv4(bit1, bit2, bit3, bit4);
      else
        aIstream.setstate(std::ios::failbit);
