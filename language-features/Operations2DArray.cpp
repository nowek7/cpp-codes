...
class Array2D
  ...
  public:
    ...
    constexpr T *data() noexcept { return theContainer.data(); }
    constexpr T const *data() const noexcept { return theContainer.data(); }

    constexpr T &at(const size_t aRowIdx, const size_t aColIdx)
      {
      return theContainer.at(aRowIdx * C + aColIdx);
      }

    constexpr T const &at(const size_t aRowIdx, const size_t aColIdx) const
      {
      return theContainer.at(aRowIdx * C + aColIdx);
      }

    constexpr T &operator()(const size_t aRowIdx, const size_t aColIdx)
      {
      return theContainer[aRowIdx * C + aColIdx];
      }

    constexpr T const &operator()(const size_t aRowIdx, const size_t aColIdx) const
      {
      return theContainer[aRowIdx * C + aColIdx];
      }
    ...
    constexpr size_t size(const int aRank) const
      {
      if (aRank == 1)
        return R;
      else if (aRank == 2)
    ...
    const_iterator begin() const { return theContainer.data(); }
    const_iterator end() const { return theContainer.data() + theContainer.size(); }
    iterator begin() { return theContainer.data(); }
    iterator end() { return theContainer.data() + theContainer.size(); }
  };
