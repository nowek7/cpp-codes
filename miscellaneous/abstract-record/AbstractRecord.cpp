...
Record **generate(const int &aSize)
  {
  ...
  if (aSize > 0)
    ...
    for (int i = 0; i < aSize; i++)
      records[i] = new Record(integerRange(randomEngine), MIN_REAL_NUMBER + i, static_cast<char>(letterRange(randomEngine)));
...
void removeRecords(Record **aRecords, const int &aSize)
  {
  if (aRecords)
    {
    for (int i = 0; i < aSize; i++)
...
void sort(Record **aRecords, const int &aSize)
  {
  // Using bubble sort;
...
int countSigns(Record **aRecords, const int &aCount, const char &aSign)
  {
  int count = 0;
  if (aRecords)
    {
...
int main()
  ...
  for (int i = 0; i < PRINT_COUNT; i++)
    std::cout << records[i]->theIntegerNumber << "," << records[i]->theRealNumber << "," << records[i]->theLetter << std::endl;
