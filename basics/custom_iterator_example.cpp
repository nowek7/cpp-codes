...
struct Tag
{
  ...
  Tag(const int id = 0, const std::string& name = ""s):
    id(id),
    name(name)
  {
  }
...
public:
  ...
  typedef dummyArrayIterator<const Type, SIZE> constant_iterator;
...
int main()
{
...

  {
    ...
    std::transform(array.begin(), array.end(), array.begin(), [](const int e) {
      return e * 2;
    });
...

  {
    ...
    auto lp = [](const dummyArray<int, 5>& container) {
