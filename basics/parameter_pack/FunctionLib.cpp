...
struct fround
{
  typename std::enable_if<std::is_floating_point<T>::value, T>::type operator()(const T& value) const
  {
...
{
  {
    ...
    auto m = FunctionLib::mapf(
      [](const std::pair<std::string, int> kvp) {
...

  {
    ...
    auto count = FunctionLib::foldl(
      [](const int s, const std::pair<std::string, int> kvp) {
        return s + kvp.second;
      },
      words,
      0);
...

  {
    ...
    auto s = FunctionLib::foldl(std::plus<>(),
                                FunctionLib::mapf(
                                  [](const int i) {
                                    return i + i;
                                  },
